...
public class RegularCustomerChain implements CustomerChain {
	
	private DiscountChain chain1;
	private AmountValidation amountValidation = new AmountValidation();
	
	public RegularCustomerChain() {
		this.chain1 = new TwentyPercentDiscountRC();
		DiscountChain chain2 = new TenPercentDiscountRC();
		DiscountChain chain3 = new NoDiscountRC();
	...
	public void invokeDiscountChain(Amount amount) throws Exception {
			...
			if(amountValidation.validateAmount(amount.getPurchaseAmount())) {
				System.out.println("Purchase amount is "+amount.getPurchaseAmount());
	
			  RegularCustomerChain regularCustomerChain = new RegularCustomerChain();
			  regularCustomerChain.chain1.discount(amount);
